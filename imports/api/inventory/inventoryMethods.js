...
Meteor.methods({
    ...
    inventoryCreateItem (data) {
        ...
        console.log("inventoryCreateItem: start", { currentLoggedInUser, itemName: data.itemName, quantity: data.quantity, unitOfMeasure: data.unitOfMeasure })
        check(data.itemName, String);
        check(data.quantity, Number);
        check(data.unitOfMeasure, String);

        // if not log in throw an error
        if(!currentLoggedInUser) throw new Meteor.Error('not logged in', "Hey men! Please login first before proceeding.")

        // Insert data on Inventory collection
        return Inventory.insert({ itemName: data.itemName, quantity: data.quantity, unitOfMeasure: data.unitOfMeasure, addedAt: new Date() });
    },
