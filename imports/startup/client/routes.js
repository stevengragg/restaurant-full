...
// Route function
const route = name => FlowRouter.route(`/${name}`, { name: configureName(name), action: () => {
  if(!Meteor.userId()) {
    FlowRouter.go('signin');
    return;
  }
  //If not admin then access denied for this pages
  if(!isAdmin() && (name === 'users' || name === 'vendors' || name === 'warehouses' || name === 'inventory-adjustments')){
...
Tracker.autorun(() => {
  ...
  console.log('FlowRouter route', { path: FlowRouter.current().path, params: FlowRouter.current().params, queryParams: FlowRouter.current().queryParams });
});
...
FlowRouter.route('/products/new-product', { name: 'new_product', action: renderRouteName });