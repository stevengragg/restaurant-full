...
Template.warehouses.helpers({
    warehouseContactTypeHelper() { return types; },
    warehouseContacts () { 
    ...
    isViewingWarehouse() {
        ...
        if(getSessionisViewingWarehouse.bool) return getSessionisViewingWarehouse.warehouseId;
        else return false;
    },
...
Template.warehouses.events({
    ...
    'click .js-create-warehouse-contact' (event,target) {
        ...
        Meteor.call('warehouseCreate', {warehouseName,shortName,address}, (err,res) => {
            if(err) { alert(err.reason); return;}
            if(!res.success) { alert(ise); return; }
            else { window.$('#warehouseModal').modal('hide') }
        })
    },
    'click .js-remove-warehouse' (event,target) {
        console.log(this._id);
        const answer = confirm("Are you sure that you want to remove this warehouse information?");
        if(answer) { Meteor.call('warehouseRemove', { warehouseId: this._id}, (err) => { if(err) alert(err.reason); }) }
        else return;   
    },
