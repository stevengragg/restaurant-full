...
Template.vendors.helpers({
    vendorContactTypeHelper() { return types; },
    vendorContacts () { 
        return Vendors.find({});
    },
    isViewingVendor() {
        ...
        if(getSessionisViewingVendor.bool) return getSessionisViewingVendor.vendorId;
        else return false;
    },
    showVendor(vendorId) {
...
Template.vendors.events({
    ...
    'click .js-create-vendor-contact' (event,target) {
        ...
        console.log('vendorObj',{type,name,address,email,mobile, internalNotes});
        Meteor.call('vendorCreate', {type,name,address,email,mobile, internalNotes}, (err,res) => {
            if(err) { alert(err.reason); return;}
            if(!res.success) { alert(ise); return; }
            else { $('#vendorsModal').modal('hide') }
        })
    ...
    'click .js-remove-vendor' (event,target) {
        ...
        const answer = confirm("Are you sure that you want to remove this contact?");
        if(answer) { Meteor.call('vendorRemove', { vendorId: this._id}, (err) => { if(err) alert(err.reason); }) }
        else return;   
    },
